...
export default function Projects(props) {
    ...
    const handleScrollOnPointer = (e) => {
        ...
        if (e.pointerType == "touch") {
            ...
            if (e.type == "pointerup" || e.type == "pointercancel") {
                ...
                if ((startTouchPos.current - e.clientX) > (currentSlide.scrollWidth * thresholdToSwitchSlide) && (startTouchPos.current - e.clientX) < currentSlide.scrollWidth) {
                    // console.log("__T__H__R__E__SH NEXT");
                    next();
                } else if ((e.clientX - startTouchPos.current) > (currentSlide.scrollWidth * thresholdToSwitchSlide) && Math.abs(e.clientX - startTouchPos.current) < currentSlide.scrollWidth) {
                    // console.log("__T__H__R__E__SH PREV");
                    prev();
                } else {
                    setSlideNum(Math.round((scrollContainer.scrollLeft) / currentSlide.scrollWidth) + 1);
                    // console.log("__T__H__R__E__SH MORE THAN ONE");
                }
                setNeedRedraw(true);
            }
        }
    ...
    useEffect(() => {
        ...
        function scrollThis(timeStamp) {
            ...
            const transitionDuration = 700 * Math.max(Math.min(Math.abs(targetScrollLeft - curElmPos) / (scrollWidth / 2), 1), 0.3),
    ...
    return (
        <section id="projects" className="flex flex-col justify-start w-full pl-[30px] sm:pl-[50px] lg:pl-[80px] overflow-hidden " >
            <h2 className="text-[22px] lg:text-[44px] font-semibold mt-[80px] lg:mt-[130px] ">Selected Portfolio Projects</h2>
            <div className="relative h-fit ">
                ...
                <div ref={refScrollContainer} tabIndex={0} id="DOMScrollContainer" className={`flex flex-row gap-[20px] pr-[calc(100dvw-30px-300px)] sm:pr-[calc(100dvw-50px-440px)] lg:pr-[calc(100dvw-80px-870px)] overflow-hidden scroll-py-[20px] snap-always ${snapClass} snap-mandatory transition touch-none `}
...
function SingleProject(props) {
    ...
    return (
        <div id={`Item-${props.ind}`} key={props.ind * 13} ref={props.refSlide} className={`snap-start ${props.ind == props.slideNum ? props.opacityElm : "opacity-100"} `} >
            <div className={`relative flex flex-row justify-center items-center w-[300px] sm:w-[440px] lg:w-[870px] sm:min-h-[340px] lg:min-h-[550px] min-h-[189px]  mt-[15px] lg:mt-[30px] bg-cover `} >
                <button className="peer absolute z-[2] top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 flex flex-row justify-center items-center gap-[5px] lg:gap-[15px] text-white text-[12px] lg:text-[22px] font-normal tracking-wide capitalize rounded-full bg-myblue/80 w-[103px] h-[30px] lg:w-[181px] lg:h-[50px] hover:bg-myblue/100 active:bg-myblue/100 hover:outline-[#14C6B7] focus:outline-[#14C6B7] "
                    onClick={() => window.location.assign(`/portfolio/${props.name.replace(" ", "_").toLowerCase()}`)}
                >{props.button}
                </button>
                <div className="peer absolute z-[1] top-0 left-0 size-full hover:bg-my-grad-acc focus:bg-my-grad-acc hover:opacity-30 peer-hover:bg-my-grad-acc peer-hover:opacity-30" />
                <Image
                    className="object-cover bg-myblue peer-hover:blur-[2px] "
                    src={props.image}
            ...
            <div className="flex">
                <Link href={`/portfolio/${props.name.replace(" ", "_").toLowerCase()}`}>
                    <p className="min-h-[21px] lg:min-h-[32px] text-[18px] lg:text-[28px] font-semibold mt-[20px] lg:mt-[30px] cursor-pointer hover:text-[#14C6B7] leading-none">{props.name}</p>
                </Link>
            </div>
        </div >
    )
